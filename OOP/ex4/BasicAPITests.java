...
public class BasicAPITests {
    ...
    private static String[] wordsData3;
    ...
    private void testOnData(SimpleSet obj, String[] words)
    {
        ...
        for(int i = 0; i < 17; i++) Assert.assertFalse(obj.add(words[i])); // no duplicates
        for(int i = 0; i < 17; i++) Assert.assertTrue(obj.delete(words[i]));
        for(int i = 0; i < 17; i++) Assert.assertFalse(obj.delete(words[i])); //deleted
        for(int i = 0; i < 17; i++) Assert.assertFalse(obj.contains(words[i])); //deleted
        Assert.assertEquals(obj.size(), words.length - 17);
    }
